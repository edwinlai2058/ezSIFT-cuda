                    ...
                    std::list<MatchPair> &match_list)
{
    ...
    for (int i = 0; i < (int)kpt_list1.size(); ++i) {
        // We need to advance iterator to i-th element because list is not random access
        // This is inefficient but mimics original behavior for baseline
        auto kpt1 = kpt_list1.begin();
        std::advance(kpt1, i);
        ...
        // Position of the matched feature.
        int r2 = 0, c2 = 0;
        for (auto kpt2 = kpt_list2.begin(); kpt2 != kpt_list2.end(); kpt2++) {
            float score = 0;
            float *descr2 = kpt2->descriptors;
            float dif;

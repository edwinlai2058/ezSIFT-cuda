...
int build_dog_pyr(std::vector<Image<float>>& gpyr, std::vector<Image<float>>& dogPyr, int nOctaves, int nDogLayers) {
    ...
    for (int i = 0; i < nOctaves; i++) {
        ...
        for (int j = 0; j < nDogLayers; j++) {
            ...
            index = 0;
            while (index++ < w * h) *(dstData++) = *(srcData2++) - *(srcData1++);
        }
    }
...
int detect_keypoints(std::vector<Image<float>>& dogPyr, std::vector<Image<float>>& grdPyr,
    ...
    for (int i = 0; i < nOctaves; i++) {
        ...
        size_t start_size = kpt_list.size();
        ...
        for (int j = 1; j < nDogLayers - 1; j++) {
            ...
            for (int r = SIFT_IMG_BORDER; r < h - SIFT_IMG_BORDER; r++) {
                for (int c = SIFT_IMG_BORDER; c < w - SIFT_IMG_BORDER; c++) {
                    ...
                    bool bExtrema =
                        (val >= contr_thr && val > highData[index - w - 1] && val > highData[index - w] &&
                         val > highData[index - w + 1] && val > highData[index - 1] && val > highData[index] &&
                         val > highData[index + 1] && val > highData[index + w - 1] && val > highData[index + w] &&
                         val > highData[index + w + 1] && val > currData[index - w - 1] && val > currData[index - w] &&
                         val > currData[index - w + 1] && val > currData[index - 1] && val > currData[index + 1] &&
                         ...
                         val > lowData[index + w - 1] && val > lowData[index + w] &&
...
#define INTERPOLATE_ORI_HIST
        ...
        size_t end_size = kpt_list.size();
        printf("Octave %d keypoints: %lu\n", i, end_size - start_size);
    }
...
int sift_cpu(const Image<unsigned char>& image, std::list<SiftKeypoint>& kpt_list, bool bExtractDescriptors) {
    ...
    printf("Baseline SIFT_IMG_DBL: %d, firstOctave: %d\n", SIFT_IMG_DBL, firstOctave);
    ...
    // Dump gpyr[0]
    FILE* fp_g0 = fopen("gpyr0_baseline.bin", "wb");
    if(fp_g0) {
    ...
    // Dump dogPyr[1] (Octave 0, Layer 1)
    FILE* fp = fopen("dog_baseline.bin", "wb");
    if(fp) {
        fwrite(dogPyr[1].data, sizeof(float), dogPyr[1].w * dogPyr[1].h, fp);
        fclose(fp);
    }

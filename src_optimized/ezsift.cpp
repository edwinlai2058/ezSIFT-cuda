...
void init_sift_parameters(bool doubleFirstOctave, float contrast_threshold, float edge_threshold,
                          float match_NDDR_threshold) {
...
int build_octaves(const Image<unsigned char>& image, std::vector<Image<unsigned char>>& octaves, int firstOctave,
                  int nOctaves) {
    int w = image.w;
    int h = image.h;
    if (firstOctave == -1) {
...
int gaussian_blur(const Image<float>& in_image, Image<float>& out_image, std::vector<float> coef1d) {
    // START OPTIMIZATION: Use CUDA implementation
    return gaussian_blur_cuda(in_image, out_image, coef1d);
    // END OPTIMIZATION
}
...
int row_filter_transpose(float* src, float* dst, int w, int h, float* coef1d, int gR) {
    // START OPTIMIZATION: Hoist memory allocation out of the loop
    #pragma omp parallel
    {
        // Allocate buffer once per thread (raw pointer to avoid zero-init overhead)
        float* row_buf = new float[w + gR * 2];
        
        // Use static schedule to improve cache locality and reduce false sharing 
        // since threads write to adjacent memory locations if dispersed (False Sharing).
        // With static, each thread gets a contiguous block of rows.
        ...
        for (int r = 0; r < h; r++) {
            ...
            for (int c = 0; c < w; c++) {
                ...
                // 3. Write to transposed location
...
int build_gaussian_pyramid(std::vector<Image<unsigned char>>& octaves, std::vector<Image<float>>& gpyr, int nOctaves,
                           int nGpyrLayers) {
    int nLayers = nGpyrLayers - 3;
    std::vector<std::vector<float>> gaussian_coefs = compute_gaussian_coefs(nOctaves, nGpyrLayers);

    // START OPTIMIZATION: Hybrid CPU/GPU Pyramid Build
    // build_gaussian_pyramid_cuda(octaves[0].to_float(), gpyr, nOctaves, nGpyrLayers, gaussian_coefs);
    gpyr.resize(nOctaves * nGpyrLayers);
    int w, h;
    for (int i = 0; i < nOctaves; i++) {
        w = octaves[i].w;
        h = octaves[i].h;
        for (int j = 0; j < nGpyrLayers; j++) {
            if (i == 0 && j == 0) {
                ...
                // downsample
                gpyr[i * nGpyrLayers].init(w, h); 
                downsample_cuda(gpyr[(i - 1) * nGpyrLayers + nLayers], gpyr[i * nGpyrLayers]);
            } else {
                gpyr[i * nGpyrLayers + j].init(w, h);
                // Note: gaussian_blur calls gaussian_blur_cuda (wrapper)
...
int build_dog_pyr(std::vector<Image<float>>& gpyr, std::vector<Image<float>>& dogPyr, int nOctaves, int nDogLayers) {
    ...
    for (int i = 0; i < nOctaves; i++) {
        ...
        for (int j = 0; j < nDogLayers; j++) {
            ...
            // Input images from Gaussian Pyramid
            Image<float>& im1 = gpyr[row_start + j];
            Image<float>& im2 = gpyr[row_start + j + 1];
            Image<float>& out = dogPyr[i * nDogLayers + j];

            // Use CUDA subtraction
            subtract_cuda(im1, im2, out);
        }
    }
...
int build_grd_rot_pyr(std::vector<Image<float>>& gpyr, std::vector<Image<float>>& grdPyr,
                      std::vector<Image<float>>& rotPyr, int nOctaves, int nLayers) {
...
float compute_orientation_hist(const Image<float>& image, SiftKeypoint& kpt, float*& hist) {
    ...
    for (int i = -win_radius; i <= win_radius; i++)  // rows
    {
        ...
        for (int j = -win_radius; j <= win_radius; j++)  // columns
        {
            ...
            dr = get_pixel_f(data, w, h, r + 1, c) - get_pixel_f(data, w, h, r - 1, c);
            dc = get_pixel_f(data, w, h, r, c + 1) - get_pixel_f(data, w, h, r, c - 1);
            ...
            weight = expf(-1.0f * ((i - d_kptr) * (i - d_kptr) + (j - d_kptc) * (j - d_kptc)) / (2.0f * sigma * sigma));
...
#define USE_SMOOTH1 1
    ...
    for (int i = 0; i < nBins; i++) {
        hist[i] = (TMPHIST(i - 2) + TMPHIST(i + 2)) * 1.0f / 16.0f + (TMPHIST(i - 1) + TMPHIST(i + 1)) * 4.0f / 16.0f +
...
float compute_orientation_hist_with_gradient(const Image<float>& grdImage, const Image<float>& rotImage,
...
#define USE_SMOOTH1 1
    ...
    // Smooth the histogram. Algorithm comes from OpenCV.
    hist[0] =
        (tmpHist[0] + tmpHist[2]) * 1.0f / 16.0f + (tmpHist[0] + tmpHist[1]) * 4.0f / 16.0f + tmpHist[0] * 6.0f / 16.0f;
    hist[1] =
        (tmpHist[0] + tmpHist[3]) * 1.0f / 16.0f + (tmpHist[0] + tmpHist[2]) * 4.0f / 16.0f + tmpHist[1] * 6.0f / 16.0f;
    hist[nBins - 2] = (tmpHist[nBins - 4] + tmpHist[nBins - 1]) * 1.0f / 16.0f +
                      (tmpHist[nBins - 3] + tmpHist[nBins - 1]) * 4.0f / 16.0f + tmpHist[nBins - 2] * 6.0f / 16.0f;
    hist[nBins - 1] = (tmpHist[nBins - 3] + tmpHist[nBins - 1]) * 1.0f / 16.0f +
                      (tmpHist[nBins - 2] + tmpHist[nBins - 1]) * 4.0f / 16.0f + tmpHist[nBins - 1] * 6.0f / 16.0f;
...
// Keypoint detection.
int detect_keypoints(std::vector<Image<float>>& dogPyr, std::vector<Image<float>>& grdPyr,
                     std::vector<Image<float>>& rotPyr, int nOctaves, int nDogLayers,
                     std::list<SiftKeypoint>& kpt_list) {
    // This function is obsolete in the optimized pipeline.
    return 0;
}
...
bool refine_local_extrema(std::vector<Image<float>>& dogPyr, int nOctaves, int nDogLayers, SiftKeypoint& kpt) {
    ...
    for (; i < SIFT_MAX_INTERP_STEPS; i++) {
        ...
        dxy = (get_pixel_f(currData, w, h, r + 1, c + 1) - get_pixel_f(currData, w, h, r + 1, c - 1) -
               get_pixel_f(currData, w, h, r - 1, c + 1) + get_pixel_f(currData, w, h, r - 1, c - 1)) *
              0.25f;
        dxs = (get_pixel_f(highData, w, h, r, c + 1) - get_pixel_f(highData, w, h, r, c - 1) -
               get_pixel_f(lowData, w, h, r, c + 1) + get_pixel_f(lowData, w, h, r, c - 1)) *
              0.25f;
        dys = (get_pixel_f(highData, w, h, r + 1, c) - get_pixel_f(highData, w, h, r - 1, c) -
               get_pixel_f(lowData, w, h, r + 1, c) + get_pixel_f(lowData, w, h, r - 1, c)) *
              0.25f;
...
int extract_descriptor(std::vector<Image<float>>& grdPyr, std::vector<Image<float>>& rotPyr, int nOctaves,
    ...
    for (std::list<SiftKeypoint>::iterator kpt = kpt_list.begin(); kpt != kpt_list.end(); kpt++) {
        ...
        for (int i = top; i <= bottom; i++)  // rows
        {
            for (int j = left; j <= right; j++)  // columns
            {
                ...
                float vrco110, vrco111, vrco100, vrco101, vrco010, vrco011, vrco000, vrco001;
...
int sift_cpu(const Image<unsigned char>& image, std::list<SiftKeypoint>& kpt_list, bool bExtractDescriptors) {
    ...
    printf("SIFT_IMG_DBL: %d, firstOctave: %d\n", SIFT_IMG_DBL, firstOctave);
    ...
    // START OPTIMIZATION: Full GPU Pipeline
    
    // 1. Prepare Host Pyramids (Allocated but empty content)
    std::vector<Image<float>> gpyr(nOctaves * nGpyrLayers);
    std::vector<Image<float>> dogPyr(nOctaves * nDogLayers);
    
    // We need to initialize dimensions first so we can allocate host memory to copy back to later
    for (int i = 0; i < nOctaves; i++) {
        int w = octaves[0].w >> i;
        int h = octaves[0].h >> i;
        for (int j = 0; j < nGpyrLayers; j++) {
    ...
    // 2. Upload Base Image
    float* d_base;
    int w_base = octaves[0].w;
    int h_base = octaves[0].h;
    
    // Convert octave[0] to float for upload
    Image<float> base_float = octaves[0].to_float();
    // 2. Upload to Device
    allocate_device_image(&d_base, w_base, h_base);
    upload_image(d_base, base_float.data, w_base, h_base);  
    
    // 3. Run GPU Pipeline
    std::vector<float*> d_gpyr_ptrs;
    std::vector<float*> d_dogPyr_ptrs;
    std::vector<std::vector<float>> gaussian_coefs = compute_gaussian_coefs(nOctaves, nGpyrLayers);
    
    // A. Build Gaussian Pyramid on Device
    build_gaussian_pyramid_cuda_device(d_base, w_base, h_base, d_gpyr_ptrs, nOctaves, nGpyrLayers, gaussian_coefs);
    
    // B. Build DoG Pyramid on Device
    build_dog_pyr_cuda_device(d_gpyr_ptrs, d_dogPyr_ptrs, nOctaves, nDogLayers, w_base, h_base);
    
    // C. Detect Keypoints on Device
    // In Lowe's paper, |D(x)|<0.03 will be rejected.
    float contr_thr = 0.8f * SIFT_CONTR_THR;
    std::vector<SiftKeypointCoords> coords = detect_keypoints_cuda_device(d_dogPyr_ptrs, nOctaves, nDogLayers, w_base, h_base, contr_thr);
    
    // 4. Download Pyramids to Host (for Refinement/Orientation steps)
    // Copy gpyr
    for(size_t k = 0; k < d_gpyr_ptrs.size(); k++) {
        download_image(d_gpyr_ptrs[k], gpyr[k]);
    }
    // Copy dogPyr
    for(size_t k = 0; k < d_dogPyr_ptrs.size(); k++) {
        download_image(d_dogPyr_ptrs[k], dogPyr[k]);
    }
    
    // 5. Cleanup GPU
    free_pyramid_device(d_gpyr_ptrs);
    free_pyramid_device(d_dogPyr_ptrs);
    free_device_mem(d_base);
    
    // END OPTIMIZATION: GPU Pipeline Complete
    
    // Dump gpyr[0]
    FILE* fp_g0 = fopen("gpyr0_optimized.bin", "wb");
    if(fp_g0) {
        fwrite(gpyr[0].data, sizeof(float), gpyr[0].w * gpyr[0].h, fp_g0);
        fclose(fp_g0);
    }
    ...
    for (int i = 0; i < nOctaves; i++) {
        for (int j = 0; j < nGpyrLayers; j++) {
            ...
            write_float_pgm(fgpyr, gpyr[i * nGpyrLayers + j].data, gpyr[i * nGpyrLayers + j].w,
                            gpyr[i * nGpyrLayers + j].h, 1);
        }
    }
#endif

    // Build gradient and rotation pyramids (On CPU using downloaded gpyr)
    ...
    // 6. Process Keypoints on Host (Refinement & Orientation)
    // Reuse logic from detect_keypoints but iterate over GPU candidates
    int nBins = SIFT_ORI_HIST_BINS;
    ...
    for (const auto& coord : coords) {
        ...
        kpt.octave = i;
        kpt.layer = j;
        kpt.ri = (float)coord.r;
        kpt.ci = (float)coord.c;
        ...
        float max_mag = compute_orientation_hist_with_gradient(
            grdPyr[i * nGpyrLayers + kpt.layer], rotPyr[i * nGpyrLayers + kpt.layer], kpt, hist);

        float threshold = max_mag * SIFT_ORI_PEAK_RATIO;

        for (int ii = 0; ii < nBins; ii++) {
            // Use 3 points to fit a curve and find the accurate location
            ...
            if (currHist > lhist && currHist > rhist && currHist > threshold) {
                // Parabola fitting
                float parabola_a, parabola_b, parabola_c;
                parabola_a = 0.5f * (lhist - 2.0f * currHist + rhist);
                parabola_b = 0.5f * (rhist - lhist);
                parabola_c = currHist;
                float bin_location = -parabola_b / (2.0f * parabola_a);
                float fit_peak = parabola_a * bin_location * bin_location + parabola_b * bin_location + parabola_c;
                
                kpt.mag = fit_peak;
                kpt.ori = (ii + bin_location + 0.5f) * _2PI / nBins;

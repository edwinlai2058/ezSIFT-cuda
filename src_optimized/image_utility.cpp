...
#include <vector> // 記得 include vector

int match_keypoints(std::list<SiftKeypoint> &kpt_list1,
                    std::list<SiftKeypoint> &kpt_list2,
                    std::list<MatchPair> &match_list)
{
    // 1. Convert list to vector for parallel traversal
    std::vector<std::list<SiftKeypoint>::iterator> kpt1_vec;
    kpt1_vec.reserve(kpt_list1.size());
    for (auto it = kpt_list1.begin(); it != kpt_list1.end(); ++it) {
        kpt1_vec.push_back(it);
    }

    // 2. Parallel match finding
    // Optimization: Use per-thread match lists to avoid critical section
    #pragma omp parallel
    {
        std::list<MatchPair> thread_local_matches;
        
        #pragma omp for schedule(dynamic)
        for (int i = 0; i < (int)kpt1_vec.size(); ++i) {
            std::list<SiftKeypoint>::iterator kpt1 = kpt1_vec[i];
            ...
            for (kpt2 = kpt_list2.begin(); kpt2 != kpt_list2.end(); kpt2++) {
                ...
                float *descr2 = kpt2->descriptors;
                float dif;
                for (int k = 0; k < DEGREE_OF_DESCRIPTORS; k++) {
                    dif = descr1[k] - descr2[k];
                    score += dif * dif;
                }
    ...
    #endif
            {
                ...
                // Add to local list (no lock needed)
                thread_local_matches.push_back(mp);
            }
        }
        
        // Merge local list to global list with critical section
        // This is much faster as it only runs once per thread, not once per match
        #pragma omp critical
        {
            match_list.splice(match_list.end(), thread_local_matches);
        }
    }
